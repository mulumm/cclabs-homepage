...
const ProjectPage = () => {
  ...
  // Read URL hash to support header-driven status selection
  useEffect(() => {
    const handleHash = () => {
      try {
  ...
  // Highlight 섹션을 제거했으므로 slice 개수를 좀 더 늘려도 좋습니다 (예: 5개)
              ...
              <div className="bg-transparent">
                ...
                <div className="space-y-2">
                  {loading ? <p className="text-gray-400">Loading...</p> : govProgress.length === 0 ? <p className="text-gray-400">No projects</p> : govProgress.map((p) => (
                    <button key={p.title} onClick={() => setSelectedProject(p.title)} className="w-full text-left group/item">
                      {/* 배경 제거, border-b 추가 */}
                      <div className="py-4 border-b border-white/5 group-hover/item:border-[#4dabf7]/50 transition-colors">
                        <h4 className="font-bold text-white group-hover/item:text-[#4dabf7] transition-colors mb-1 truncate">{p.title}</h4>
                        <div className="flex justify-between items-center text-sm">
                           <p className="text-gray-400 truncate w-2/3">{p.participant || ''}</p>
              ...
              <div className="bg-transparent">
                ...
                <div className="space-y-2">
                  {loading ? <p className="text-gray-400">Loading...</p> : govCompleted.length === 0 ? <p className="text-gray-400">No completed</p> : govCompleted.map((p) => (
                    <button key={p.title} onClick={() => setSelectedProject(p.title)} className="w-full text-left group/item">
                       <div className="py-4 border-b border-white/5 group-hover/item:border-gray-500/50 transition-colors">
                        <h4 className="font-bold text-gray-300 group-hover/item:text-white transition-colors mb-1 truncate">{p.title}</h4>
                         <div className="flex justify-between items-center text-sm">
                           <p className="text-gray-500 truncate w-2/3">{p.participant || ''}</p>
          ...
          <div className="group relative bg-[#151b2b] rounded-3xl p-8 md:p-10 border border-white/5 hover:border-green-500/30 transition-all duration-300">
             <div className="flex items-center justify-between mb-8">
              ...
              <button 
                ...
                className="flex items-center gap-2 text-green-400 hover:text-white transition-colors group"
              >
                <span className="font-medium">View All</span>
                <ArrowRight size={18} className="transform group-hover:translate-x-1 transition-transform" />
              </button>
              ...
              <div className="bg-transparent">
                ...
                <div className="space-y-2">
                  {loading ? <p className="text-gray-400">Loading...</p> : indProgress.length === 0 ? <p className="text-gray-400">No projects</p> : indProgress.map((p) => (
                    <button key={p.title} onClick={() => setSelectedProject(p.title)} className="w-full text-left group/item">
                       <div className="py-4 border-b border-white/5 group-hover/item:border-green-500/50 transition-colors">
                        <h4 className="font-bold text-white group-hover/item:text-green-400 transition-colors mb-1 truncate">{p.title}</h4>
                         <div className="flex justify-between items-center text-sm">
                           <p className="text-gray-400 truncate w-2/3">{p.participant || ''}</p>
                           <p className="text-gray-500 font-mono text-xs">{formatDate(p.start_date)}</p>
                        </div>
